...
class RGI(RGIBase):
    ...
    def validate_inputs(self):
        ...
        if self.output_file == self.input_sequence and self.clean:
            logger.error(
                "output path same as input, must specify "
        ...
        logger.info(
            "{} => {}".format(self.input_sequence, filetype.guess(self.input_sequence))
        )
        ...
        if self.threads > os.cpu_count():
            logger.error(
                "Argument num_threads illegal value, expected (>=1 and =<{}):  given `{}`)".format(
    ...
    def clean_directory(self, directory, basename_output_file):
        ...
        for f in files:
            if os.path.basename(self.input_sequence) + ".temp" in f and os.path.isfile(
                f
            ):
                self.remove_file(f)
            if os.path.basename(self.input_sequence) + ".fai" in f and os.path.isfile(
                f
            ):
                self.remove_file(f)
        ...
        if os.path.exists(f):
            try:
                ...
                if (
                    self.keep == True
                    and (
                        f.find("temp.contig.fsa") != -1
                        or f.find("temp.contigToORF.fsa") != -1
                    )
                    and os.path.splitext(os.path.basename(f))[1][1:].strip() in ["fsa"]
                ):
                    pass
                else:
    ...
    def process_protein(self):
        ...
        xml_file = os.path.join(
            self.working_directory, "{}.temp.blastRes.xml".format(file_name)
        )
    ...
    def filter_process(self):
        ...
        filter_obj = Filter(
            ...
            self.input_sequence,
            self.blast_results_xml_file,
            os.path.join(self.dp, "card.json"),
            os.path.basename(self.input_sequence),
            self.output_file,
            self.threads,
            self,
        )
