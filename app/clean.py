...
def clean():
    ...
    for f in files:
        remove_directory(f)
        if os.path.isfile(f) and os.path.splitext(os.path.basename(f))[1][
            1:
        ].strip() in ["adraft", "xml", "fsa", "draft", "pyc", "log"]:
            os.remove(f)

        if os.path.isdir(f) == False:
            if os.path.isfile(f) == True and os.path.splitext(os.path.basename(f))[1][
                1:
            ].strip() in ["py", "md"]:
                pass
            else:
    ...
    for datafile in data_files:
        if os.path.isfile(datafile) and os.path.basename(datafile) not in [
            "card.json",
            ".gitignore",
            "__init__.py",
        ]:
    ...
    for dbfile in db_files:
        if os.path.isfile(dbfile) and os.path.basename(dbfile) not in [
            ".gitignore",
            "__init__.py",
        ]:
...
def create_parser():
    parser = argparse.ArgumentParser(
        prog="rgi clean",
        description="{} - {} - Clean".format(APP_NAME, SOFTWARE_VERSION),
    )
    parser.add_argument(
        "--local",
        dest="local_database",
