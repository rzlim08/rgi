...
def main(args):
    ...
    logger.info(
        "Creating temp directory with prefix {}".format(
            os.path.join(os.getcwd(), "rgi_autoload_")
        )
    )
    ...
    os.system(
        "wget {verbose} -O {data} --no-check-certificate https://card.mcmaster.ca/download/0/broadstreet-v{card_cannonical_version}.tar.bz2".format(
            verbose=verbose, data=data, card_cannonical_version=card_cannonical_version
        )
    )
    ...
    os.system(
        "wget {verbose} -O {variants} --no-check-certificate https://card.mcmaster.ca/download/6/prevalence-v{card_variants_version}.tar.bz2".format(
            verbose=verbose,
            variants=variants,
    ...
    os.system(
        "tar xf {variants} -C {card_variants}".format(
            variants=variants, card_variants=card_variants
        )
    )
    os.system(
        "gunzip {verbose} {card_variants}/*.gz".format(
    ...
    os.system(
        "rgi wildcard_annotation --input_directory {card_variants} --version {card_variants_version} --card_json {card_data}/card.json".format(
    ...
    os.system(
        "rgi clean {debug} {local_database}".format(
    ...
    if args.clean:
        ...
        os.system(
            "rm card_database_v{card_cannonical_version}.fasta".format(
                card_cannonical_version=card_cannonical_version
            )
        )
        os.system(
            "rm card_database_v{card_cannonical_version}_all.fasta".format(
                card_cannonical_version=card_cannonical_version
            )
        )
        os.system(
            "rm wildcard_database_v{card_variants_version}.fasta".format(
                card_variants_version=card_variants_version
            )
        )
        os.system(
            "rm wildcard_database_v{card_variants_version}_all.fasta".format(
                card_variants_version=card_variants_version
            )
        )
...
def create_parser():
    parser = argparse.ArgumentParser(
        prog="rgi auto_load",
        description="{} - {} - Automatic Load".format(APP_NAME, SOFTWARE_VERSION),
    )
    parser.add_argument(
        "--local",
        dest="local_database",
