...
def combine_variant_sequences(f1, f2, f3, f4):
    os.system(
        "cat {fasta_one} {fasta_two} {fasta_three} {fasta_four} > {prev_fasta}".format(
            fasta_one=f1,
            fasta_two=f2,
            fasta_three=f3,
            fasta_four=f4,
            prev_fasta=os.path.join(
...
def split_variant_sequences(index, fasta):
    ...
    with open(os.path.join(working_directory, "both.fasta"), "w") as both:
        with open(os.path.join(working_directory, "plasmid.fasta"), "w") as plasmids:
            with open(os.path.join(working_directory, "chr.fasta"), "w") as chrs:
                with open(
                    os.path.join(working_directory, "contig.fasta"), "w"
                ) as contigs:
    ...
    with open(os.path.join(working_directory, "species.fasta"), "w") as single_path:
        with open(os.path.join(working_directory, "genus.fasta"), "w") as single_genus:
            with open(
                os.path.join(working_directory, "multi.fasta"), "w"
            ) as multi_path:
                for entry in SeqIO.parse(fasta, "fasta"):
                    prev_id = entry.id.split(":")[1].split("|")[0]
...
def count_kmers(k, threads):
    ...
    # Counts kmers
    os.system(
        "jellyfish count --mer-len {k} --threads {threads} --size 2G --output {jf_out} {fasta}".format(
            k=k,
            jf_out=os.path.join(working_directory, "species.temp.jf"),
    ...
    os.system(
        "jellyfish dump --column --tab --output {name} {db_path}".format(
            name=os.path.join(working_directory, species_kmers),
    ...
    os.system(
        "jellyfish count --mer-len {k} --threads {threads} --size 2G --output {jf_out} {fasta}".format(
            k=k,
            jf_out=os.path.join(working_directory, "genus.temp.jf"),
    ...
    os.system(
        "jellyfish dump --column --tab --output {name} {db_path}".format(
            name=os.path.join(working_directory, genus_kmers),
    ...
    os.system(
        "jellyfish count --mer-len {k} --threads {threads} --size 2G --output {jf_out} {fasta}".format(
            k=k,
            jf_out=os.path.join(working_directory, "multi.temp.jf"),
    ...
    os.system(
        "jellyfish dump --column --tab --output {name} {db_path}".format(
            name=os.path.join(working_directory, multi_kmers),
    ...
    os.system(
        "jellyfish count --mer-len {k} --threads {threads} --size 2G --output {jf_out} {fasta}".format(
            k=k,
            jf_out=os.path.join(working_directory, "both.temp.jf"),
    ...
    os.system(
        "jellyfish dump --column --tab --output {name} {db_path}".format(
            name=os.path.join(working_directory, both_kmers),
    ...
    os.system(
        "jellyfish count --mer-len {k} --threads {threads} --size 2G --output {jf_out} {fasta}".format(
            k=k,
            jf_out=os.path.join(working_directory, "plasmid.temp.jf"),
    ...
    os.system(
        "jellyfish dump --column --tab --output {name} {db_path}".format(
            name=os.path.join(working_directory, plasmid_kmers),
    ...
    os.system(
        "jellyfish count --mer-len {k} --threads {threads} --size 2G --output {jf_out} {fasta}".format(
            k=k,
            jf_out=os.path.join(working_directory, "chr.temp.jf"),
    ...
    os.system(
        "jellyfish dump --column --tab --output {name} {db_path}".format(
            name=os.path.join(working_directory, chr_kmers),
...
def main(args):
    ...
    if not args.skip:
        try:
            ...
            exit(
                "ERROR: cannot locate prevalence sequences and/or index-for-model-sequences.txt in directory"
            )
        else:
            ...
            split_variant_sequences(
                index,
                os.path.join(working_directory, "nucleotide_prevalence_all.fasta"),
            )
            print("DONE \n")
    else:
        pass
    ...
    variant_sequences = os.path.join(
        working_directory, "nucleotide_prevalence_all.fasta"
    )

    print("-- CREATING KMER SETS --")
    app.make_kmer_json.make_json(
        plasmid_file,
        chr_file,
        both_file,
        genus_file,
    ...
    os.system(
        "cat {card} {variants} > card_and_prevalence.fasta".format(
            card=args.card_fasta, variants=variant_sequences
        )
    )
    ...
    os.system(
        "jellyfish count --mer-len {k} --threads {threads} --size 2G --output {jf_out} {fasta}".format(
            k=k,
            jf_out=os.path.join(working_directory, "all_amr.temp.jf"),
    ...
    os.system(
        "jellyfish dump --column --tab --output {name} {db_path}".format(
            name=os.path.join(working_directory, amr_kmers),
...
def create_parser():
    parser = argparse.ArgumentParser(
        prog="rgi kmer_build",
        description="{} - {} - Kmer Build \n\nBuilds the kmer sets for CARD*kmers".format(
    ...
    parser.add_argument("-k", dest="k", required=True, help="k-mer size (e.g., 61)")
    parser.add_argument(
        "--skip",
        dest="skip",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="number of threads (CPUs) to use (default={})".format(1),
    )
    parser.add_argument(
        "--batch_size",
        dest="batch_size",
