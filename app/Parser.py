            ...
            try:
                ...
                for key in hsp:
                    if hsp[key]["model_id"] in snps[orf]:
                        snps[orf][hsp[key]["model_id"]].append(
                            hsp[key]["snp"]["original"]
                            + str(hsp[key]["snp"]["position"])
                            + hsp[key]["snp"]["change"]
                            + ":"
                            + hsp[key]["model_name"]
                        )
                    else:
                        snps[orf][hsp[key]["model_id"]] = [
                            hsp[key]["snp"]["original"]
                            + str(hsp[key]["snp"]["position"])
                            + hsp[key]["snp"]["change"]
                            + ":"
    ...
    for orf, hsp in j.items():
        if isinstance(hsp, dict):
            ...
            if "ARO_category" in j[orf][best_hsp]:
                check_for_all_classifications(
                    "category_aro_class_name", j[orf][best_hsp]["ARO_category"]
                )
                    ...
                    for c in criteria:
                        if j[orf][best_hsp]["type_match"] == c:
                            ...
                            if (
                                ...
                                in j[orf][best_hsp]["ARO_category"][key]
                            ):
                                if (
                                    j[orf][best_hsp]["ARO_category"][key][
                                        "category_aro_class_name"
                                    ]
                                    == "Drug Class"
                                ):
                                    ...
                                    if not not no_rm:
                                        if (
                                            len(no_rm[j[orf][best_hsp]["type_match"]])
                                            > 0
                                        ):
                                            rm[j[orf][best_hsp]["type_match"]][
                                                "unclassified"
                                            ] = no_rm[j[orf][best_hsp]["type_match"]]
    ...
    for dc in m:
        ...
        for crit in dc:
            if bool(dc[crit]) is False:
                if not any(
                    crit in item.values() for item in finaldrugclass["children"]
                ):
                    ...
                    for hit in dc[crit][i]:
                        ...
                        for orf in j:
                            for hsp in j[orf]:
                                if (
                                    orf in hit.keys()
                                    and hsp in hit.values()
                                    and j[orf][hsp]["type_match"] == crit
                                ):
                                    innerdict[
                                        j[orf][hsp]["model_id"]
                                        + "__gene_header__"
                                    ...
                                    ndict["pass_bitscore"] = j[orf][hsp][
                                        "pass_bitscore"
                                    ]
                                    ndict["bit_score"] = j[orf][hsp]["bit_score"]
                                    ndict["Predicted_Protein"] = j[orf][hsp][
                                        "orf_prot_sequence"
                                    ]
                                    ndict["CARD_Protein_Sequence"] = j[orf][hsp][
                                        ...
                                        for i in s[orf]:
                                            if i == j[orf][hsp]["model_id"]:
                                                ndict[
                                                    ...
                                                    x.split(":")[0] for x in s[orf][i]
                                                )
                                            else:
                                                ndict["Other_SNPs"] = ", ".join(
                                                    x + ":" + i for x in s[orf][i]
                                                )
                                    else:
...
def create_parser():
    parser = argparse.ArgumentParser(
        prog="rgi parser",
        description="{} - {} - Parser \n\nCreates categorical .json files RGI wheel visualization. An input .json file containing the RGI results must be input.".format(
            APP_NAME, SOFTWARE_VERSION
        ),
        formatter_class=RawTextHelpFormatter,
    )
    parser.add_argument(
        "-i", "--input", dest="input", required=True, help="RGI results in a .json file"
    )
    parser.add_argument(
        "-o",
        "--output",
        dest="output",
        default="RGIResults",
        help="Name/identifier for the output categorical .json files",
    )
    parser.add_argument(
        "--include_loose",
        dest="loose",
    ...
    parser.add_argument(
        ...
        dest="type",
        default="contig",
        help="type of input sequence: contig, protein or read",
    )
    return parser
...
def calc_number_of_hits(m, f):
    ...
    count_json = {
        "Perfect": len(perfect_list),
        "Strict": len(strict_list),
        "Loose": len(loose_list),
    }
    return count_json
