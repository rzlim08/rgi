...
def write_fasta_annotation_file(data, version, ncbi, all_model_type=False):
    ...
    output_file = os.path.join(
        working_directory, "card_database_v{}.fasta".format(version)
    )
    if all_model_type is True:
        ...
        output_file = os.path.join(
            working_directory, "card_database_v{}_all.fasta".format(version)
        )
            ...
            if i.isdigit():
                ...
                if data[i]["model_type_id"] in selected_model_types:
                    ...
                    if "ARO_category" in data[i]:
                        for c in data[i]["ARO_category"]:
                            if "category_aro_class_name" in data[i]["ARO_category"][c]:
                                if data[i]["ARO_category"][c][
                                    "category_aro_class_name"
                                ] in ["Drug Class"]:
                                    drug_class.append(
                                        (
                                            "{}".format(
                                                data[i]["ARO_category"][c][
                                                    "category_aro_name"
                                                ]
                                            )
                                        )
                    ...
                    try:
                        for seq in data[i]["model_sequences"]["sequence"]:
                            if ncbi is True:
                                ...
                                header = "gb|{ncbi}|ARO:{ARO_accession}|ID:{model_id}|Name:{ARO_name}".format(
                                    ...
                                    ncbi=data[i]["model_sequences"]["sequence"][seq][
                                        "dna_sequence"
                                    ]["accession"],
                                )
                            else:
                                header = "ARO:{}|ID:{}|Name:{}|NCBI:{}".format(
                                    ...
                                    data[i]["model_sequences"]["sequence"][seq][
                                        "dna_sequence"
                                    ]["accession"],
                                )

                            sequence = data[i]["model_sequences"]["sequence"][seq][
                                "dna_sequence"
                            ]["sequence"]
                            fout.write(">{}\n".format(header))
                            fout.write("{}\n".format(sequence))
                        ...
                        logger.debug(e)
                        logger.warning(
                            "No model sequences for model ({}, {}). Omitting this model and keep running.".format(
                                data[i]["model_id"], data[i]["model_name"]
                            )
                        )
...
def create_parser():
    parser = argparse.ArgumentParser(
        prog="rgi card_annotation",
        description="{} - {} - CARD Annotation \n\nCreates card annotations for RGI BWT from card.json".format(
            APP_NAME, SOFTWARE_VERSION
        ),
        formatter_class=RawTextHelpFormatter,
    )
    parser.add_argument(
        "-i", "--input", dest="input", required=True, help="card.json file"
    )
    parser.add_argument(
        "--ncbi",
        dest="ncbi",
        action="store_true",
        help="adds ncbi accession to FASTA headers (default: False)",
    )
    parser.add_argument(
