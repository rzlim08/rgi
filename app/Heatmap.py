...
class Heatmap(object):
    ...
    def __init__(
        self, input, classification, frequency, output, cluster, display, debug
    ):
        self.input = input
    ...
    def create_categories(self, class_dict, df):
        ...
        for model in class_dict:
            if len(class_dict[model]) > 1:
                df = pd.concat(
                    [df, pd.DataFrame([df.loc[model]] * (len(class_dict[model]) - 1))]
                )
    ...
    def draw_categories(self, ax1, ranges, cat_list, ax0, display, df):
        ...
        label_bb = ax0.yaxis.get_ticklabels()[
            0
        ].get_window_extent()  # get height of labels
        ...
        ax1.plot([1, 1], [0, ranges[0]], lw=10, color=pal[3])
        ax1.text(
            0.5,
            (ranges[0] / 2),
            cat_list[0],
    ...
    def write_csv(self, category, df, class_series, output):
        ...
        if self.classification in col_list:
            reordered_columns = [self.classification] + [
                ...
                if i != col_list.index(self.classification)
            ]
        else:
            logger.warning(
                "Couldn't create csv of heatmap matrix. "
    ...
    def run(self):
        ...
        for thing in files:
            ...
            if thing.endswith(".json") and os.path.isfile(
                file_path
                                    ...
                                    for entry in value[hsp]["ARO_category"]:
                                        ...
                                        # Drug classes classification
                                            ...
                                            if (
                                                ...
                                                drug_class[value[hsp]["model_name"]] = [
                                                    value[hsp]["ARO_category"][entry][
                                                        "category_aro_name"
                                                    ]
                                                ]
        ...
        for e in excluded:
            print(
                "NOTE: %s excluded because it is missing complete categorization information."
                % (e)
            )
        ...
        if not genes:
            logger.error(
                "Error: No data recovered from JSONs, cannot build heatmap. "
        ...
        if self.classification:
            ...
            complete_class_df = pd.concat(
                [class_df1, class_df2, class_df3], axis=1, sort=True
            )
            ...
            if self.classification == "drug_class":
                ...
                complete_class_df = (
                    complete_class_df.set_index(
                        ["resistance_mechanism", "gene_family"], append=True
                    )["drug_class"]
                    .apply(pd.Series)
                    .stack()
                )
                complete_class_df = complete_class_df.reset_index()
                complete_class_df.columns = [
                    "model_name",
                ...
                complete_class_df = (
                    complete_class_df.set_index(
                        ["drug_class", "gene_family"], append=True
                    )["resistance_mechanism"]
                    .apply(pd.Series)
                    .stack()
                )
                ...
                complete_class_df = (
                    complete_class_df.set_index(
                        ["drug_class", "resistance_mechanism"], append=True
                    )["gene_family"]
                    .apply(pd.Series)
                    .stack()
                )
            ...
            complete_class_df = complete_class_df.sort_values(
                by=[self.classification, "model_name"]
            )
            ...
            if self.frequency:
                ...
                if self.cluster == "samples":
                    ...
                    logger.error(
                        "Error: Unable to cluster genes because the categorization option was chosen. No heatmap will be generated. Closing program now."
                    )
                    exit()

                # Set the figure size
                fig_width, fig_length, fig, figsize = self.get_figure_dimensions(
                    jsons, unique_ids
                )
                ...
                while True:
                    if self.get_axis_size(fig, ax0)[1] > 150:
                        ...
                        ax0, ax1, ax2, gs = self.create_plot("cf", 4)
                    if (
                        self.get_axis_size(fig, ax0)[0]
                        < (self.get_axis_size(fig, ax0)[1]) / 3
                    ):
                        # print('LESS THAN 1/3')
                        fig_length = fig_length / 2
                        figsize = (fig_width, fig_length)
                ...
                # Create the heatmap
                g = sns.heatmap(
                    df, cmap=custom_cmap, cbar=False, ax=ax0, norm=norm
                )  # linewidth=0.5
                ...
                if self.cluster == "samples":
                    print(
                        "Output file {fn}: AMR genes categorised by {c} and only unique "
                        ...
                        "antibiotic resistance ontology (ARO).".format(
                            fn=file_name, c=classification
                        )
                    )
                else:
                    print(
                        "Output file {fn}: AMR genes categorised by {c} and "
                        ...
                        "one {c} category in the antibiotic resistance ontology (ARO).".format(
                            fn=file_name, c=classification
                        )
                    )
            ...
            # Categories, but no frequency
                ...
                if self.cluster == "samples":
                    print(
                        "Output file {n}: AMR genes categorised by {c} and "
        ...
        # No categories
            ...
            if self.frequency:
                ...
                if self.cluster == "samples":
                    print(
                        "Output file %s: AMR genes are listed in alphabetical order and unique "
                    ...
                    print(
                        "Output file %s: AMR genes have been clustered hierarchically "
                    ...
                    print(
                        "Output file %s: AMR genes and unique resistome profiles "
                ...
                # Create the heatmap
                g = sns.heatmap(
                    df, cmap=custom_cmap, cbar=False, norm=norm
                )  # linewidth=0.5
                plt.setp(g.yaxis.get_ticklabels(), rotation=0, fontsize="xx-large")
                plt.setp(g.xaxis.get_ticklabels(), rotation=90, fontsize="xx-large")
                g.set_ylabel(" ")
                ...
                if self.cluster == "samples":
                    print(
                        "Output file %s: AMR genes are listed in alphabetical order "
                    ...
                    print(
                        "Output file %s: AMR genes and samples have been clustered hierarchically "
                        "(see SciPy documentation). Yellow represents a perfect hit, teal represents a strict hit, purple "
                        "represents no hit." % (file_name)
                    )
                else:
                    print(
                        "Output file %s: Yellow represents a perfect hit, "
                        "teal represents a strict hit, purple represents no hit."
                        % (file_name)
                    )
