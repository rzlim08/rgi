...
def get_taxon_kmers(
    single_file, multi_file, variant_sequences, index_file, k, type, threads, batch_size
):
    ...
    for ind in range(len(l)):
        process = multiprocessing.Process(
            target=query_kmers,
            args=(l[ind], ind, variant_sequences, output, batch_size),
        )
        process.start()
...
def make_json(
    ...
    with open(
        os.path.join(os.path.join(os.getcwd()), "{k}_kmer_db.json".format(k=k)), "w"
    ) as out:
        json.dump(final, out)
...
def run():
    parser = argparse.ArgumentParser(description="Creates a kmer catalogue")
    parser.add_argument(
        "-p",
        dest="plasmid",
        required=True,
    ...
    parser.add_argument("-k", dest="k", required=True, help="kmer length")
    parser.add_argument(
        "-n",
        "--threads",
        dest="threads",
