...
class MainBase(object):
    def __init__(self, api=False):
        ...
        parser = argparse.ArgumentParser(
            prog="rgi",
            description="{} - {}".format(APP_NAME, SOFTWARE_VERSION),
            epilog=SOFTWARE_SUMMARY,
            usage=USAGE,
        )
        parser.add_argument(
            "command",
    ...
    def main_args(self):
        ...
        parser.add_argument(
            "--split_prodigal_jobs",
            dest="split_prodigal_jobs",
            action="store_true",
            help="run multiple prodigal jobs simultaneously for contigs in a fasta file (default: False)",
        )
        return parser
    ...
    def tab_args(self):
        parser = argparse.ArgumentParser(
            prog="rgi tab",
            description="{} - {} - Tab-delimited".format(APP_NAME, SOFTWARE_VERSION),
        )
        parser.add_argument(
    ...
    def kmer_query_args(self):
        parser = argparse.ArgumentParser(
            prog="rgi kmer_query",
            description="{} - {} - Kmer Query \n\nTests sequenes using CARD*kmers".format(
        ...
        parser.add_argument(
            "-k", "--kmer_size", dest="k", required=True, help="length of k"
        )
        parser.add_argument(
            "-m",
            "--minimum",
    ...
    def bwt_args(self):
        ...
        parser.add_argument(
            "-1", "--read_one", required=True, help="raw read one (qc and trimmed)"
        )
        ...
        parser.add_argument(
            ...
            "--aligner",
            default="kma",
            choices=["kma", "bowtie2", "bwa"],
            help="select read aligner (default=kma)",
        )
        parser.add_argument(
            "-n",
            "--threads",
        ...
        parser.add_argument(
            ...
            dest="output_file",
            required=True,
            help="name of output filename(s)",
        )
        parser.add_argument(
            "--debug",
            dest="debug",
        ...
        parser.add_argument(
            ...
            action="store_true",
            help="include wildcard (default=False)",
        )
        parser.add_argument(
            "--include_other_models",
            dest="include_other_models",
            action="store_true",
        ...
        parser.add_argument(
            ...
            dest="include_baits",
            action="store_true",
            help="include baits (default=False)",
        )
        parser.add_argument(
            "--mapq",
            dest="mapq",
            help="filter reads based on MAPQ score (default=False)",
        )
        parser.add_argument(
            "--mapped",
            dest="mapped",
    ...
    def tm_args(self):
        ...
        parser.add_argument(
            "-o", "--output_file", dest="output_file", help="output_file"
        )
        parser.add_argument(
            "-t",
            "--filter_temperature",
            dest="filter_temperature",
            default=65,
            help="desired melting temperature (default=65).",
        )
        parser.add_argument(
            "--clean", dest="clean", action="store_true", help="removes temporary files"
        )
        parser.add_argument(
            "--debug", dest="debug", action="store_true", help="debug mode"
        )
    ...
    def heatmap_args(self):
        ...
        parser.add_argument(
            ...
            "--category",
            dest="classification",
            choices=("drug_class", "resistance_mechanism", "gene_family"),
    ...
    def galaxy_args(self):
        parser = argparse.ArgumentParser(
            prog="rgi galaxy",
            description="{} - {} - Galaxy project wrapper".format(
                APP_NAME, SOFTWARE_VERSION
            ),
            epilog=GALAXY_PROJECT_WRAPPER,
        )
        parser.add_argument(
            "--galaxy_database",
            help="path to CARD data and blast databases",
            required=True,
        )
        parser.add_argument(
            "--debug", dest="debug", action="store_true", help="debug mode"
    ...
    def database_args(self):
        ...
        parser.add_argument(
            ...
            action="store_true",
            required=True,
            help="prints data version number",
        )
        parser.add_argument(
            "--local",
            dest="local_database",
        ...
        if os.path.isfile(indecies_directory) == True:
            with open(indecies_directory) as json_file:
                ...
                if args.all == True:
                    ...
                    if (
                        "model_type_used" in json_data["card_canonical"].keys()
                        or "model_type_used" in json_data["card_variants"].keys()
                    ):
