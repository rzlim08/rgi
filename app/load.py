...
def get_card_annotation(
    args_card_annotation, args_local_database, filename="card_reference.fasta"
):
...
def main(args):
    ...
    version_file = os.path.join(
        get_location(args.local_database), "loaded_databases.json"
    )
    if os.path.isfile(version_file):
        ...
        loaded_databases = {
            "card_json": {"data_version": "N/A", "model_type_used": []},
            "card_canonical": {"data_version": "N/A", "model_type_used": []},
            "card_variants": {"data_version": "N/A", "model_type_used": []},
            "card_kmers": {"kmer_sizes": []},
        }
    ...
    if args.card_json is not None:
        ...
        loaded_databases["card_json"]["data_version"] = get_card_json_version(
            os.path.join(get_location(args.local_database), "card.json")
        )
        loaded_databases["card_json"]["model_type_used"] = [
            "homolog",
            "variant",
            "rRNA",
            "overexpression",
    ...
    if args.card_annotation is not None:
        load_reference_card_only(
            args.local_database, args.card_annotation, "card_reference.fasta"
        )
        loaded_databases["card_canonical"]["model_type_used"] = ["homolog"]
        loaded_databases["card_canonical"]["data_version"] = loaded_databases[
            "card_json"
        ]["data_version"]
    else:
        # check if card annotation is already loaded
    ...
    if args.card_annotation_all_models is not None:
        load_reference_card_only(
            args.local_database,
            args.card_annotation_all_models,
            "card_reference_all.fasta",
        )
        loaded_databases["card_canonical"]["model_type_used"] = [
            "homolog",
            "variant",
            "rRNA",
            "overexpression",
        ...
        card_annotation_all = get_card_annotation(
            args.card_annotation_all_models,
            args.local_database,
            "card_reference_all.fasta",
        )
    ...
    if args.wildcard_index is not None:
        load_file(
            args.local_database, args.wildcard_index, "index-for-model-sequences.txt"
        )
    ...
    if args.wildcard_annotation is not None and args.card_annotation is not None:
        ...
        load_reference_card_and_wildcard(
            args.local_database,
            card_annotation,
            args.wildcard_annotation,
    ...
    # CARD[All] + WILD[ALL]
    if (
        args.wildcard_annotation_all_models is not None
        and args.card_annotation_all_models is not None
    ):
        load_reference_card_and_wildcard(
            args.local_database,
            card_annotation_all,
            args.wildcard_annotation_all_models,
            "card_wildcard_reference_all.fasta",
        )
        loaded_databases["card_variants"]["data_version"] = args.wildcard_version
        loaded_databases["card_variants"]["model_type_used"] = [
            "homolog",
            "variant",
            "rRNA",
            "overexpression",
    ...
    if (
        args.baits_index is not None and args.baits_annotation is not None
    ):  # and args.card_annotation is not None:
        logger.info("adding index and fasta for baits")
        # load index
        load_file(
            args.local_database, args.baits_index, "baits-probes-with-sequence-info.txt"
        )
        # load annotation files (baits)
        load_reference_card_and_baits(
            args.local_database,
            card_annotation,
            args.baits_annotation,
            "card_baits_reference.fasta",
        )

    if (
        args.wildcard_index is not None
        and args.wildcard_annotation is not None
        and args.baits_index is not None
        and args.baits_annotation is not None
    ):
        # load annotations files for CARD, VARIANTS and BAITS
        load_reference_card_and_wilcard_and_baits(
            args.local_database,
            card_annotation,
            args.baits_annotation,
            args.wildcard_annotation,
            "card_wildcard_baits_reference.fasta",
        )
        loaded_databases["card_variants"]["data_version"] = args.wildcard_version
...
def load_reference_card_and_wilcard_and_baits(
    local_db, card_fasta_file, baits_fasta_file, wildcard_fasta_file, filename
):
    ...
    with open(os.path.join(db, filename), "w") as fout, fileinput.input(
        filenames
    ) as fin:
        for line in fin:
            fout.write(line)
...
def load_reference_card_and_baits(
    local_db, card_fasta_file, baits_fasta_file, filename
):
    db = get_location(local_db)
...
def load_reference_card_and_wildcard(
    local_db, card_fasta_file, wildcard_fasta_file, filename
):
    db = get_location(local_db)
    # check files
    ...
    with open(os.path.join(db, filename), "w") as fout, fileinput.input(
        ...
        for line in fin:
            fout.write(line)
    logger.info(
        "loaded the following files: {} annotations for 'rgi bwt'.".format(
            ",".join(filenames)
        )
    )
...
def load_file(local_db, filepath, filename, validate_json=False):
    logger.info(
        "local_db: {}, filepath: {}, filename: {},  validate_json: {}".format(
            local_db, filepath, filename, validate_json
        )
    )
    db = get_location(local_db)
...
def create_parser():
    parser = argparse.ArgumentParser(
        prog="rgi load", description="{} - {} - Load".format(APP_NAME, SOFTWARE_VERSION)
    )
    parser.add_argument(
        "-i", "--card_json", required=True, help="must be a card database json file"
    )
    parser.add_argument(
        "--card_annotation",
        required=False,
        help="annotated reference FASTA for homolog models only, created using rgi card_annotation",
    )
    parser.add_argument(
        "--card_annotation_all_models",
        required=False,
        help="annotated reference FASTA which includes all models created using rgi card_annotation",
    )
    parser.add_argument(
        "--wildcard_annotation",
        required=False,
        help="annotated reference FASTA for homolog models only, created using rgi wildcard_annotation",
    )
    parser.add_argument(
        "--wildcard_annotation_all_models",
        required=False,
        help="annotated reference FASTA which includes all models created using rgi wildcard_annotation",
    )
    parser.add_argument(
        "--wildcard_index",
        required=False,
        help="wildcard index file (index-for-model-sequences.txt)",
    )
    parser.add_argument(
        "--wildcard_version", required=False, help="specify variants version used"
    )
    parser.add_argument(
        "--baits_annotation", required=False, help="annotated reference FASTA"
    )
    parser.add_argument(
        "--baits_index",
        required=False,
    ...
    parser.add_argument(
        "--kmer_size", required=False, help="kmer size if loading kmer files"
    )
    parser.add_argument(
        "--local",
        dest="local_database",
