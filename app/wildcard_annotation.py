...
def get_model(input_directory):
    ...
    try:
        with open(
            os.path.join(input_directory, "index-for-model-sequences.txt"), "r"
        ) as ifile:
            lines = csv.reader(ifile, delimiter="\t")
            for row in lines:
                if row[0] != "prevalence_sequence_id":
...
def write_fasta_annotation_file(
    files, prev_models, annotations, version, all_model_type=False
):
    working_directory = os.getcwd()
    fasta_file = os.path.join(
        working_directory, "wildcard_database_v{}.fasta".format(version)
    )
    fastas = {}
    # homolog only
    ...
    if all_model_type is True:
        fasta_file = os.path.join(
            working_directory, "wildcard_database_v{}_all.fasta".format(version)
        )
            ...
            for record in SeqIO.parse(f, "fasta"):
                if record:
                    ...
                    header = "Prevalence_Sequence_ID:{prevalence_sequence_id}|ID:{model_id}|Name:{aro_term}|{aro_accession}".format(
                        ...
                        aro_term=annotations[Prevalence_Sequence_ID][
                            "aro_term"
                        ].replace(" ", "_"),
                        aro_accession=annotations[Prevalence_Sequence_ID][
                            "aro_accession"
                        ],
                    )
...
def create_parser():
    parser = argparse.ArgumentParser(
        prog="rgi wildcard_annotation",
        description="{} - {} - WildCARD Annotation \n\nCreates card annotations for RGI BWT from Variants or Wilcard data".format(
    ...
    parser.add_argument(
        ...
        required=True,
        help="input directory for wildcard",
    )
    parser.add_argument(
        "-v",
        "--version",
        dest="version",
        required=True,
        help="specify version downloaded for wildcard / variants",
    )
    parser.add_argument(
        "-j", "--card_json", dest="card_json", required=True, help="card.json file"
    )
    parser.add_argument(
