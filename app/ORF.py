...
class ORF(object):
    ...
    def orf_prodigal(self):
        ...
        (
            ...
            number_of_sequences,
        ) = self.min_max_sequence_length()
        logger.info(
            "minimum sequence length: {}, maximun sequence length {}, number of sequences: {}".format(
        ...
        if number_of_sequences > 1 and self.split_prodigal_jobs == True:
            ...
            cmd = "prodigal -q -m -a {trans_file} -i {input_file} -o  {output_file} -d {nuc_file} -s {potential_genes} {quality} {stdout}".format(
    ...
    def worker(self, input_fasta):
		...
		-s {wd}/{tmp_name}.temp.potentialGenes 2> /dev/null".format(
            input_fasta=input_fasta, wd=self.working_directory, tmp_name=o_f_name
        )
        # logger.debug(cmd)
        os.system(cmd)
    ...
    def prodigal_run(self, fasta, *o):
        ...
        for entry in fasta:
            ...
            tmp = os.path.join(
                self.working_directory, "{}.temp.directory".format(o_f_name)
            )
    ...
    def write_output_file(self, output, filenames):
        with open(
            os.path.join(self.working_directory, output), "a+"
        ) as fout, fileinput.input(filenames) as fin:
            for line in fin:
                fout.write(line)
    ...
    def contig_to_orf(self):
        ...
        if self.training_file is not None:
            ...
            orf_finder = pyrodigal.OrfFinder(
                meta=False, mask=True, training_info=training_info
            )
        ...
        with contextlib.ExitStack() as stack:
            # open result files
            filename = os.path.basename(self.input_file)
            trans_filename = os.path.join(
                self.working_directory, "{}.temp.contig.fsa".format(filename)
            )
            trans_file = stack.enter_context(open(trans_filename, "w"))
            nuc_filename = os.path.join(
                self.working_directory, "{}.temp.contigToORF.fsa".format(filename)
            )
            ...
            for record, genes in zip(
                records, pool.map(orf_finder.find_genes, sequences)
            ):
