...
class Baits(object):
    ...
    def __init__(self, input_file, output_file, filter_temperature, clean, debug):
        ...
        self.filtered_output = os.path.join(
            o_f_path,
            self.output_file + "_filtered_{}.json".format(self.filter_temperature),
        )
        self.clean = clean
        self.debug = debug
    ...
    def check_melt_program(self):
        ...
        if "returned non-zero exit status 1" in str(output):
            logger.error(
                "Missing program melt.pl. Please install melt.pl from: http://unafold.rna.albany.edu/?q=unafold-man-pages"
            )
            exit()
    ...
    def run(self):
        ...
        os.system(
            "{program} -n RNA {input} > {output_file}".format(
                program="melt.pl", input=self.input_file, output_file=self.output_file
            )
        )
        ...
        if self.clean == True:
            ...
            for f in files:
                if os.path.isfile(f) == True and os.path.splitext(os.path.basename(f))[
                    1
                ][1:].strip() in ["run", "plot", "ext", "ct", "dG"]:
    ...
    def melt_filter(self):
        ...
        with open(self.all_output, "r") as jfile:
            ...
            for i in data:
                if data[i]["melting_temperature (Tm)"] >= float(
                    self.filter_temperature
                ):
                    filtered_results[j] = data[i]
                    j = j + 1

        logger.info(
            "saved filtered results (tempature by {}) in {}".format(
                self.filter_temperature, self.filtered_output
            )
        )
