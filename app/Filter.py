...
class Filter(BaseModel):
    ...
    def worker(self, model_type):
        ...
        try:
            ...
            with open(
                os.path.join(
                    self.working_directory,
                    "{}.temp.{}.json".format(file_name, model_type),
                ),
                "w",
            ) as fout:
    ...
    def prepare_output(self):
        ...
        file_name = os.path.basename(self.input_sequence)
        obj = ConvertJsonToTSV(
            self.output_file,
            os.path.join(
                self.working_directory, "{}.temp.{}.json".format(file_name, "homolog")
            ),
            os.path.join(
                self.working_directory, "{}.temp.{}.json".format(file_name, "variant")
            ),
            os.path.join(
                self.working_directory,
                "{}.temp.{}.json".format(file_name, "overexpression"),
            ),
            os.path.join(
                self.working_directory, "{}.temp.{}.json".format(file_name, "rrna")
            ),
        )
    ...
    def process_rrna(self, model_type="rrna"):
        ...
        if self.input_type == "protein":
            ...
            with open(
                os.path.join(
                    self.working_directory, "{}.{}.json".format(file_name, model_type)
                ),
                "w",
            ) as fout:
    ...
    def create_db_query(self):
        ...
        xml_file = os.path.join(
            self.working_directory, "{}.blastRes.rrna.xml".format(f_name)
        )
    ...
    def run(self):
        if os.path.exists(self.xml_file):
            ...
            logger.error(
                "missing blast xml file({}). Please check if input_type: '{}' correspond with input file: '{}' or use '--low_quality' flag for short contigs to predicts partial genes.".format(
