...
class BaseModel(object):
    ...
    def get_orf_dna_sequence(self, input_file, input_type):
        ...
        if input_type in ["contig"]:
            contig_filepath = os.path.join(
                self.working_directory, filename + ".temp.contigToORF.fsa"
            )
            ...
            read_filepath = os.path.join(
                self.working_directory, filename + ".temp.read.fsa"
            )
        ...
        predicted_filepath = os.path.join(
            self.working_directory, filename + ".temp.predictedGenes.json"
        )
    ...
    def get_orf_protein_sequence(self, input_file, input_type):
        ...
        if input_type in ["contig"]:
            contig_filepath = os.path.join(
                self.working_directory, filename + ".temp.contig.fsa"
            )
            if os.stat(contig_filepath).st_size != 0:
                for record in SeqIO.parse(contig_filepath, "fasta"):
                    predicted_genes_dict[record.id] = str(record.seq)
        ...
        predicted_filepath = os.path.join(
            self.working_directory, filename + ".temp.predictedGenes.protein.json"
        )
        with open(predicted_filepath, "w") as wf:
            wf.write(pjson)
    ...
    def results(
        self, blast_results, query_id, perfect, strict, loose, include_nudge=False
    ):
        """
    ...
    def nudge_strict_to_perfect(self, strict):
        ...
        for s in strict:
            if (
                int(strict[s]["perc_identity"]) == 100
                and strict[s]["type_match"] == "Strict"
                and strict[s]["model_type_id"] in [40292]
                and self.input_type == "contig"
            ):
                ...
                if len(query) < len(reference) and query in reference:
                    ...
                    if len(partial_bases) % 3 == 0:
                        ...
                        if strict[s]["orf_strand"] == "-":
                            ...
                            orf_dna_sequence_ = (
                                str(Seq(partial_bases).reverse_complement())
                            ...
                            strict[s]["orf_end_possible"] = (
                                int(strict[s]["orf_end"]) + 1
                            )
                            orf_dna_sequence_ = (
                                partial_bases + strict[s]["orf_dna_sequence"]
                            )
                            partial_protein = str(
                                Seq(partial_bases).translate(table=11)
                            ).strip("*")
                        ...
                        if combine == strict[s]["sequence_from_broadstreet"]:
                            ...
                            strict[s][
                                "note"
                            ] = "possible complete gene, missing n-terminus"
                ...
                # reference contained within open reading frame
                    ...
                    if strict[s]["partial"] == "0":
                        ...
                        if start_codon in [
                            ...
                            "ATG",
                            "GTG",
                        ] and stop_codon in ["TAA", "TAG", "TGA"]:
                            strict[s][
                                "note"
                            ] = "possible complete gene, reference contained within open reading frame"
                            strict[s]["orf_prot_sequence_possible"] = ""

                            if strict[s]["orf_strand"] == "-":
                                strict[s]["orf_start_possible"] = strict[s]["orf_start"]
                                strict[s]["orf_end_possible"] = (
                                    int(strict[s]["orf_start"])
                                    + len(strict[s]["dna_sequence_from_broadstreet"])
                                    - 1
                                )
                            else:
                                strict[s]["orf_start_possible"] = (
                                    int(strict[s]["orf_end"])
                                    - len(strict[s]["dna_sequence_from_broadstreet"])
                                    + 1
                                )
                            ...
                            if len(strict[s]["orf_dna_sequence_possible"]) % 3 == 0:
                                orf_prot_sequence_possible = str(
                                    Seq(
                                        strict[s]["orf_dna_sequence_possible"]
                                    ).translate(table=11)
                                ).strip("*")
                                strict[s][
                                ...
                                if (
                                    orf_prot_sequence_possible
                                    == strict[s]["sequence_from_broadstreet"]
                                ):
                                    strict[s]["type_match"] = "Perfect"
                                    nudged = True
                            else:
                            ...
                            logger.warning(
                                "partial gene in card reference: {}, start_codon: {},  stop_codon: {}".format(
                                    strict[s]["ARO_name"], start_codon, stop_codon
                                )
                            )
                    else:
                        # print(json.dumps(strict[s], indent=2))
                        logger.warning(
                            "partial gene in card reference: {}".format(
                                strict[s]["ARO_name"]
                            )
                        )
                ...
                95 <= int(strict[s]["perc_identity"]) < 100
                and strict[s]["type_match"] == "Strict"
                and strict[s]["model_type_id"] in [40292]
                and self.input_type == "contig"
            ):
    ...
    def get_part_sequence(
        self, fasta_file, header, start, stop, nterminus, strand, name
    ):
        """
        ...
        if genes:
            ...
            if strand == "-":
                ...
                if nterminus == 0:
                    ...
                    return (
                        str(genes.get_spliced_seq(header, [[start, stop]])),
                        start,
                        stop,
                    )
                else:
                    return (
                        str(genes.get_spliced_seq(header, [[_start, _stop]])),
                        _start,
                        _stop,
                    )
            elif strand == "+":
    ...
    def nudge_loose_to_strict(self, loose):
        ...
        for i in loose:
            if 95 <= int(loose[i]["perc_identity"]) <= 100:
                ...
                loose[i][
                    "note"
                ] = "loose hit with at least 95 percent identity pushed strict"
