...
class BWT(object):
    ...
    def __init__(
        ...
        if self.include_baits == True:
            logger.info("baits")
            self.reference_genome_baits = os.path.join(
                self.data, "baits_reference.fasta"
            )
            self.index_directory_bowtie2_baits = os.path.join(
                self.db,
                self.indecies_directory,
                "baits_reference",
                "{}".format("bowtie2_baits"),
            )
            self.index_directory_bwa_baits = os.path.join(
                self.db,
                self.indecies_directory,
                "baits_reference",
                "{}".format("bwa_baits"),
        ...
        if self.include_wildcard == True and self.include_other_models == False:
            ...
            self.reference_genome = os.path.join(
                self.data, "card_wildcard_reference.fasta"
            )
            self.index_directory_bowtie2 = os.path.join(
                self.db,
                self.indecies_directory,
                "card_wildcard_reference",
                "{}".format("bowtie2"),
            )
            self.index_directory_kma = os.path.join(
                self.db,
                self.indecies_directory,
                "card_wildcard_reference",
                "{}".format("kma"),
            )
            self.index_directory_bwa = os.path.join(
                self.db,
                self.indecies_directory,
                "card_wildcard_reference",
                "{}".format("bwa"),
            ...
            logger.info(
                "CARD[homolog,rRNA,overexpression,variants,knockout] + WILD[homolog,rRNA,overexpression,variants,knockout]"
            )
            self.reference_genome = os.path.join(
                self.data, "card_wildcard_reference_all.fasta"
            )
            self.index_directory_bowtie2 = os.path.join(
                self.db,
                self.indecies_directory,
                "card_wildcard_reference_all",
                "{}".format("bowtie2"),
            )
            self.index_directory_kma = os.path.join(
                self.db,
            ...
            self.index_directory_kma = os.path.join(
                self.db, self.indecies_directory, "card_reference", "{}".format("kma")
            )
            self.index_directory_bwa = os.path.join(
                self.db, self.indecies_directory, "card_reference", "{}".format("bwa")
            )

        # outputs
        self.working_directory = os.path.join(os.getcwd())
        self.output_sam_file = os.path.join(
            self.working_directory, "{}.temp.sam".format(self.output_file)
        )
        self.output_sam_file_baits = os.path.join(
            self.working_directory, "{}.baits.temp.sam".format(self.output_file)
        )
        self.output_bam_file = os.path.join(
            self.working_directory, "{}.temp.bam".format(self.output_file)
        )
        self.output_bam_file_baits = os.path.join(
            self.working_directory, "{}.baits.temp.bam".format(self.output_file)
        )
        self.output_bam_sorted_file = os.path.join(
            self.working_directory, "{}.sorted.temp.bam".format(self.output_file)
        )
        self.sorted_bam_sorted_file_length_100 = os.path.join(
            self.working_directory, "{}.sorted.length_100.bam".format(self.output_file)
        )

        self.unmapped = os.path.join(
            self.working_directory, "{}.unmapped.temp.bam".format(self.output_file)
        )
        self.mapped = os.path.join(
            self.working_directory, "{}.mapped.temp.bam".format(self.output_file)
        )
        self.mapping_overall_stats = os.path.join(
            self.working_directory,
            "{}.overall_mapping_stats.txt".format(self.output_file),
        )
        self.mapping_artifacts_stats = os.path.join(
            self.working_directory,
            "{}.artifacts_mapping_stats.txt".format(self.output_file),
        )
        self.mapping_reference_stats = os.path.join(
            self.working_directory,
            "{}.reference_mapping_stats.txt".format(self.output_file),
        )
        self.mapping_baits_stats = os.path.join(
            self.working_directory,
            "{}.baits_mapping_stats.txt".format(self.output_file),
        )

        self.baits_reads_count = os.path.join(
            self.working_directory,
            "{}.baits_reads_count.temp.txt".format(self.output_file),
        )
        self.reads_baits_count = os.path.join(
            self.working_directory,
            "{}.reads_baits_count.temp.txt".format(self.output_file),
        )
        self.aro_term_reads = os.path.join(
            self.working_directory,
            "{}.aro_term_reads.temp.txt".format(self.output_file),
        )

        self.output_tab = os.path.join(
            self.working_directory, "{}.temp.txt".format(self.output_file)
        )
        self.output_tab_sequences = os.path.join(
            self.working_directory, "{}.seqs.temp.txt".format(self.output_file)
        )
        self.output_tab_coverage = os.path.join(
            self.working_directory, "{}.coverage.temp.txt".format(self.output_file)
        )
        self.output_tab_coverage_all_positions = os.path.join(
            self.working_directory,
            "{}.coverage_all_positions.temp.txt".format(self.output_file),
        )
        self.output_tab_coverage_all_positions_summary = os.path.join(
            self.working_directory,
            "{}.coverage_all_positions.summary.temp.txt".format(self.output_file),
        )
        self.model_species_data_type = os.path.join(
            self.working_directory,
            "{}.model_species_data_type.temp.txt".format(self.output_file),
        )
        self.allele_mapping_data_json = os.path.join(
            self.working_directory,
            "{}.allele_mapping_data.json".format(self.output_file),
        )
        self.allele_mapping_data_tab = os.path.join(
            self.working_directory,
            "{}.allele_mapping_data.txt".format(self.output_file),
        )
        self.gene_mapping_data_tab = os.path.join(
            self.working_directory, "{}.gene_mapping_data.txt".format(self.output_file)
        )

        self.baits_mapping_data_tab = os.path.join(
            self.working_directory,
            "{}.baits_mapping_data.temp.txt".format(self.output_file),
        )
        self.baits_mapping_data_json = os.path.join(
            self.working_directory,
            "{}.baits_mapping_data.temp.json".format(self.output_file),
        )
        self.reads_mapping_data_json = os.path.join(
            self.working_directory,
            "{}.reads_mapping_data.temp.json".format(self.output_file),
        )

        # map baits to complete genes
        self.baits_card_sam = os.path.join(
            self.working_directory, "{}.baits_card.temp.sam".format(self.output_file)
        )
        self.baits_card_bam = os.path.join(
            self.working_directory, "{}.baits_card.temp.bam".format(self.output_file)
        )
        self.baits_card_tab = os.path.join(
            self.working_directory, "{}.baits_card.temp.txt".format(self.output_file)
        )
        self.baits_card_json = os.path.join(
            self.working_directory, "{}.baits_card.temp.json".format(self.output_file)
        )
        self.baits_card_data_tab = os.path.join(
            self.working_directory,
            "{}.baits_card_data.temp.txt".format(self.output_file),
        )
        self.card_baits_reads_count_json = os.path.join(
            self.working_directory,
            "{}.card_baits_reads_count.temp.json".format(self.output_file),
        )
    ...
    def clean_files(self):
        ...
        if self.clean == True:
            basename_output_file = os.path.splitext(os.path.basename(self.output_file))[
                0
            ]
    ...
    def clean_directory(self, directory, basename_output_file):
        ...
        for f in files:
            if (
                os.path.basename(self.output_file) in f
                and ".temp" in f
    ...
    def create_index(self, index_directory, reference_genome):
        ...
        if self.aligner == "bowtie2":
            ...
            os.system(
                "bowtie2-build --quiet {reference_genome} {index_directory} --threads {threads}".format(
            ...
            if not os.path.exists(index_directory):
                ...
                logger.info("created index at {}".format(index_directory))
            os.system(
                "kma index -i {reference_genome} -o {index_directory}".format(
                    index_directory=index_directory, reference_genome=reference_genome
                )
            )
        else:
            if not os.path.exists(index_directory):
            ...
            os.system(
                "bwa index -p {index_directory} {reference_genome}".format(
                    index_directory=index_directory, reference_genome=reference_genome
                )
            )
    ...
    def align_kma(self, reference_genome, index_directory, output_sam_file):
        ...
        logger.info(
            "align reads -ipe {} {} to {}".format(
                self.read_one, self.read_two, reference_genome
            )
        )
    ...
    def align_bowtie2_unpaired(
        self, reference_genome, index_directory, output_sam_file
    ):
        """
    ...
    def align_bowtie2_baits_to_genes(
        self, reference_genome, index_directory, output_sam_file
    ):
        """
        ...
        logger.info(
            "align baits -f {} to complete genes in {}".format(
                self.reference_genome_baits, reference_genome
            )
        )
    ...
    def align_bwa_single_end_mapping(
        self, reference_genome, index_directory, output_sam_file
    ):
        """
        ...
        os.system(
            "bwa mem -M -t {threads} {index_directory} {read_one} > {output_sam_file}".format(
                threads=self.threads,
                index_directory=index_directory,
                read_one=self.read_one,
                output_sam_file=output_sam_file,
    ...
    def align_bwa_paired_end_mapping(
        self, reference_genome, index_directory, output_sam_file
    ):
        """
        ...
        os.system(
            "bwa mem -t {threads} {index_directory} {read_one} {read_two} > {output_sam_file}".format(
                threads=self.threads,
                index_directory=index_directory,
                read_one=self.read_one,
                read_two=self.read_two,
    ...
    def convert_sam_to_bam(self, input_sam_file, output_bam_file):
        ...
        """
        os.system(
            "samtools view --threads {threads} -b  {input_sam_file} > {output_bam_file}".format(
                threads=self.threads,
    ...
    def sort_bam(self):
        ...
        """
        os.system(
            "samtools sort --threads {threads} -T {output_file}.sorted -o {sorted_bam_file} {unsorted_bam_file}".format(
                threads=self.threads,
    ...
    def extract_alignments_with_length(self, length=10, map_quality=2):
        ...
        subprocess.run(
            [
                ...
                "filter",
                "-in",
                self.output_bam_sorted_file,
                "-out",
                self.sorted_bam_sorted_file_length_100,
            ]
        )
    ...
    def get_aligned(self):
        ...
        cmd = "samtools idxstats {input_bam} > {output_tab}".format(
            input_bam=self.sorted_bam_sorted_file_length_100, output_tab=self.output_tab
        )
        # logger.debug(cmd)
    ...
    def get_qname_rname_sequence(self):
        ...
        output_tab = self.output_tab_sequences
        subprocess.run(
            f"samtools view --threads {threads} {input_bam} | cut -f 1,2,3,4,5,7 | sort -s -n -k 1,1 > {output_tab}",
            shell=True,
            check=True,
        )
    ...
    def get_coverage(self):
        ...
        output_tab = self.output_tab_coverage
        subprocess.run(
            f"samtools depth {sorted_bam_file} > {output_tab}", shell=True, check=True
        )
    ...
    def get_coverage_all_positions(self):
        ...
        output_tab = self.output_tab_coverage_all_positions
        subprocess.run(
            f"bedtools genomecov -ibam {sorted_bam_file} > {output_tab}",
            shell=True,
            check=True,
        )
        ...
        output_file = self.output_tab_coverage_all_positions_summary
        subprocess.run(
            f"cat {output_tab} | awk '$2 > 0' | cut -f1,3,4,5 > {output_file}",
            shell=True,
            check=True,
        )
    ...
    def get_reads_count(self):
        ...
        with open(self.mapping_reference_stats, "w") as out:
            ...
            out.write("\n")
            out.write(
                "how many reference terms (or ARO terms): {}\n\n".format(
                    len(sequences.keys())
                )
            )
            ...
            if i.isdigit():
                ...
                if "model_sequences" in data[i]:
                    for item in data[i]["model_sequences"]["sequence"]:
                        taxa = " ".join(
                            data[i]["model_sequences"]["sequence"][item][
                                "NCBI_taxonomy"
                            ]["NCBI_taxonomy_name"].split()[:2]
                        )
                ...
                for c in data[i]["ARO_category"]:
                    if (
                        data[i]["ARO_category"][c]["category_aro_class_name"]
                    ...
                    if (
                        data[i]["ARO_category"][c]["category_aro_name"]
                        not in categories[
                            data[i]["ARO_category"][c]["category_aro_class_name"]
                        ]
                    ):
                        categories[
                            data[i]["ARO_category"][c]["category_aro_class_name"]
                        ].append(data[i]["ARO_category"][c]["category_aro_name"])
    ...
    def get_variant_details(self):
        ...
        df = dd.read_csv(
            os.path.join(self.data, "index-for-model-sequences.txt"),
            sep="\t",
            assume_missing=True,
            usecols=[
        ...
        df.to_csv(
            filename=self.model_species_data_type,
            single_file=True,
            mode="w",
            sep="\t",
    ...
    def get_baits_details(self):
        ...
        with open(
            os.path.join(self.data, "baits-probes-with-sequence-info.txt"), "r"
        ) as csvfile:
            reader = csv.reader(csvfile, delimiter=",", quotechar="|")
            for row in reader:
                if row[0] != "ProbeID":
    ...
    def get_coverage_details(self, hit_id):
        ...
        """
        sequences = {}
        sequences.update({hit_id: {"covered": 0, "uncovered": 0, "length": 0}})

        with open(self.output_tab_coverage_all_positions_summary, "r") as csvfile:
            reader = csv.reader(csvfile, delimiter="\t", quotechar="|")
            for row in reader:
                if hit_id == row[0]:
                    sequences[hit_id]["covered"] = sequences[hit_id]["covered"] + int(
                        row[1]
                    )
                    sequences[hit_id]["length"] = int(row[2])
        sequences[hit_id]["uncovered"] = (
            sequences[hit_id]["length"] - sequences[hit_id]["covered"]
        )
    ...
    def filter_count_reads(self, is_mapped="false", length=""):
        ...
        """
        read_one = os.path.join(
            self.working_directory, "{}.R1.fastq".format(self.output_file)
        )
        read_two = os.path.join(
            self.working_directory, "{}.R2.fastq".format(self.output_file)
        )
        options = ""
        ...
        extract_cmd = "samtools fastq -1 {read_one} -2 {read_two} {in_bam}".format(
            read_one=read_one, read_two=read_two, in_bam=self.mapped
        )
        os.system(extract_cmd)

        read_one_count_cmd = "awk '{c}' {read_one}".format(
            c="{s++}END{print s/4}", read_one=read_one
        )

        read_two_count_cmd = "awk '{c}' {read_two}".format(
            c="{s++}END{print s/4}", read_two=read_two
        )
    ...
    def get_stats(self):
        ...
        cmd_overall = "bamtools stats -in {} > {}".format(
            self.sorted_bam_sorted_file_length_100, self.mapping_overall_stats
        )
        ...
        subprocess.run(
            f"bamtools stats -in {input_bam} > {output_stats}", shell=True, check=True
        )
        ...
        with open(self.mapping_artifacts_stats, "w") as out:
            ...
            out.write("**********************************************\n")
            out.write("\n")
        cmd_artifacts = "samtools flagstat {} >> {}".format(
            self.sorted_bam_sorted_file_length_100, self.mapping_artifacts_stats
        )
        ...
        subprocess.run(
            f"samtools flagstat {input_bam} >> {output_stats}", shell=True, check=True
        )
    ...
    def probes_stats(self, baits_card):
        ...
        with open(self.aro_term_reads, "w") as tab_out3:
            ...
            writer.writerow(["ARO", "Number of Mapped Reads to baits"])
        ...
        with open(self.baits_reads_count, "w") as tab_out2:
            ...
            writer.writerow(["Bait", "Number of Mapped Reads"])
            for item in baits:
        ...
        with open(self.reads_baits_count, "w") as tab_out2:
            writer = csv.writer(tab_out2, delimiter="\t", dialect="excel")
            writer.writerow(["Read", "Baits"])
            for item in reads_to_baits:
                writer.writerow([item, "; ".join(reads_to_baits[item])])
                ...
                if "ARO" in row[2]:
                    ...
                    if probe_ok == True:
                        if term not in stats.keys():
                            ...
                            cmd = 'cat {} | grep -c "|{}|"'.format(
                                self.reference_genome_baits, term
                            )
                            cmd2 = 'cat {} | grep -c "|{}|"'.format(
                                self.reads_baits_count, term
                            )
                            stats[term] = {
                                "aro_name": name,
                                "total_baits": int(self.count_probes(cmd)),
                                "read_count": int(self.count_probes(cmd2)),
                                "mapped_baits": {probe: len(baits[probe])},
                            }
                        else:
                            if probe not in stats[term]["mapped_baits"].keys():
                                stats[term]["mapped_baits"].update(
    ...
    def get_model_id(self, models_by_accession, alignment_hit):
        model_id = ""
        if (
            alignment_hit[0:22] == "Prevalence_Sequence_ID"
            or alignment_hit[0:4] == "ARO:"
        ):
            ...
            try:
                ...
                logger.warning(
                    "missing aro accession: {} for alignment {} -> {}".format(
                        accession, alignment_hit, e
                    )
                )
        return model_id
    ...
    def get_mutation_details(self):
        # *.sam.temp.vcf.gz
        results = {}
        mutation_file_compressed = os.path.join(
            self.working_directory, "{}.temp.sam.temp.vcf.gz".format(self.output_file)
        )
    ...
    def get_read_coverage(self):
        read_coverage_file = os.path.join(
            self.working_directory, "{}.temp.sam.temp.res".format(self.output_file)
        )
    ...
    def get_consensus_sequence(self):
        consensus_sequence_file = os.path.join(
            self.working_directory, "{}.temp.sam.temp.fsa".format(self.output_file)
        )
        ...
        try:
            ...
            for a in alignments:
                ...
                if a["mrnm"] != "=" and a["mrnm"] not in mate_pair:
                    if (
                        "ARO:{}".format(models[model_id]["ARO_accession"])
                        not in a["mrnm"]
                    ):
            ...
            if "Prevalence_Sequence_ID" in alignment_hit:
                ...
                if model_id in variants.keys():
                    ...
                    for s in variants[model_id]:
                        prevalence_sequence_id = alignment_hit.split("|")[0].split(":")[
                            -1
                        ]
                        ...
                        for accession in variants[model_id][prevalence_sequence_id]:
                            ...
                            if (
                                variants[model_id][prevalence_sequence_id][accession][
                                    "data_type"
                                ]
                                not in observed_data_types
                            ):
                                observed_data_types.append(
                                    variants[model_id][prevalence_sequence_id][
                                        accession
                                    ]["data_type"]
                                )
                            ...
                            if (
                                ...
                                not in observed_in_pathogens
                            ):
                                observed_in_pathogens.append(
                                    variants[model_id][prevalence_sequence_id][
                                        accession
                        ...
                        if "Prevalence_Sequence_ID" in alignment_hit:
                            ...
                            try:
                                reference_allele_source = "In silico {rgi_criteria} {percent_identity}% identity".format(
                                    rgi_criteria=variants[model_id][
                                        prevalence_sequence_id
                                    ][_accession]["rgi_criteria"],
                                    percent_identity=variants[model_id][
                                        prevalence_sequence_id
                                    ][_accession]["percent_identity"],
                                )
                                ...
                                logger.warning(
                                    "missing key with Prev_id: {}, Exception: {}, Database: {} for model_id: {}".format(
                                        prevalence_sequence_id, e, database, model_id
                                    )
                                )
            ...
            elapsed = stop - start
            logger.info(
                "time lapsed: {} - {}".format(format(elapsed, ".3f"), alignment_hit)
            )
            ...
            (
                ...
                average_bait_coverage,
                bait_coverage_coefficient_of_variation,
            ) = self.baits_reads_counts(models[model_id]["ARO_accession"])
            ...
            if self.aligner == "kma":
                try:
                    ...
                    consensus_sequence_dna = "{}".format(
                        consensus_sequence[alignment_hit].seq
                    )
                    ...
                    if trailing_bases:
                        input_seq = (
                            "".join([input_seq, "NN"])
                            if trailing_bases == 1
                            else "".join([input_seq, "N"])
                        )

                    try:
                        consensus_sequence_protein = Seq.translate(
                            input_seq, to_stop=False, table=11, cds=True
                        )
                    except Exception as e:
                        consensus_sequence_protein = Seq.translate(
                            input_seq, to_stop=False, table=11, cds=False
                        )
                    ...
                    logger.warning(
                        "model with id : {}, has few mapped reads to make consensus sequence skipping: {}".format(
                            model_id, e
                        )
                    )
                    return {}
            ...
            return {
                ...
                "percent_coverage": {
                    "covered": format(
                        float(
                            coverage[alignment_hit]["covered"]
                            / coverage[alignment_hit]["length"]
                        )
                        * 100,
                        ".2f",
                    ),
                    "uncovered": format(
                        float(
                            coverage[alignment_hit]["uncovered"]
                            / coverage[alignment_hit]["length"]
                        )
                        * 100,
                        ".2f",
            ...
            logger.warning(
                "missing model with id : {}, Exception: {}".format(model_id, e)
            )

    def async_print(self, msg, start, stop, elapsed):
        logger.debug(
            "{} ::: parent process: {} -> process id: {} ====|{}|{}|{}".format(
                msg, os.getppid(), os.getpid(), start, stop, elapsed
            )
        )
        ...
        for r in summary:
            if r:
                ...
                if r[index] not in mapping_summary.keys():
                    ...
                    mapping_summary[r[index]]["aro_accession"].append(
                        r["aro_accession"]
                    )
                    mapping_summary[r[index]]["model_type"].append(r["model_type"])
                    mapping_summary[r[index]]["database"].append(r["database"])
                    mapping_summary[r[index]]["observed_in_genomes"].append(
                        r["observed_in_genomes"]
                    )
                    mapping_summary[r[index]]["observed_in_plasmids"].append(
                        r["observed_in_plasmids"]
                    )
                    ...
                    mapping_summary[r[index]][
                        "range_of_reference_allele_source"
                    ].append(r["range_of_reference_allele_source"])
                    mapping_summary[r[index]]["unmapped"].append(r["reads"]["unmapped"])
                    mapping_summary[r[index]]["all"].append(r["reads"]["all"])
                    mapping_summary[r[index]]["percent_coverage"].append(
                        r["percent_coverage"]["covered"]
                    )
                    mapping_summary[r[index]]["length_coverage"].append(
                        r["length_coverage"]["covered"]
                    )
                    mapping_summary[r[index]]["mapq_average"].append(r["mapq_average"])
                    mapping_summary[r[index]]["reference_sequence_length"].append(
                        r["reference"]["sequence_length"]
                    )
                    mapping_summary[r[index]]["number_of_mapped_baits"].append(
                        r["number_of_mapped_baits"]
                    )
                    mapping_summary[r[index]][
                        "number_of_mapped_baits_with_reads"
                    ].append(r["number_of_mapped_baits_with_reads"])
                    mapping_summary[r[index]]["average_bait_coverage"].append(
                        r["average_bait_coverage"]
                    )
                    mapping_summary[r[index]][
                        "bait_coverage_coefficient_of_variation"
                    ].append(r["bait_coverage_coefficient_of_variation"])

                    for m in r["mate_pair"]:
                        if m not in ["*"]:
                            arr = m.split("|")
                            if len(arr) == 4:
                                mapping_summary[r[index]]["mate_pair"].append(
                                    "{}".format(m.split("|")[2].split(":")[1])
                                )
                            elif len(arr) == 7:
                                mapping_summary[r[index]]["mate_pair"].append(
                                    "{}".format(m.split("|")[5])
                    ...
                    if (
                        r["observed_in_genomes"]
                        not in mapping_summary[r[index]]["observed_in_genomes"]
                    ):
                        mapping_summary[r[index]]["observed_in_genomes"].append(
                            r["observed_in_genomes"]
                        )
                    if (
                        r["observed_in_plasmids"]
                        not in mapping_summary[r[index]]["observed_in_plasmids"]
                    ):
                        mapping_summary[r[index]]["observed_in_plasmids"].append(
                            r["observed_in_plasmids"]
                        )
        ...
        with open(self.gene_mapping_data_tab, "w") as tab_out:
            ...
            for i in mapping_summary:
                ...
                if len(mapping_summary[i]["percent_coverage"]) > 0:
                    average_percent_coverage = sum(
                        map(float, mapping_summary[i]["percent_coverage"])
                    ) / len(mapping_summary[i]["percent_coverage"])

                if len(mapping_summary[i]["length_coverage"]) > 0:
                    average_length_coverage = sum(
                        map(float, mapping_summary[i]["length_coverage"])
                    ) / len(mapping_summary[i]["length_coverage"])

                if len(mapping_summary[i]["mapq_average"]) > 0:
                    average_mapq = sum(
                        map(float, mapping_summary[i]["mapq_average"])
                    ) / len(mapping_summary[i]["mapq_average"])

                mate_pairs = []
                mp = {
                    item: mapping_summary[i]["mate_pair"].count(item)
                    for item in mapping_summary[i]["mate_pair"]
                }
                for k in mp:
                    if k != i.replace(" ", "_"):
                ...
                # identity range
                min_identity = float(
                    min(mapping_summary[i]["range_of_reference_allele_source"])
                )
                max_identity = float(
                    max(mapping_summary[i]["range_of_reference_allele_source"])
                )
                identity_range = ""
                ...
                if self.include_other_models is False:
                    ...
                    if "protein homolog model" in mapping_summary[i]["model_type"]:
                        ...
                        writer.writerow(
                            [
                                ...
                                format(
                                    sum(map(float, mapping_summary[i]["mapped"])), ".2f"
                                ),
                                format(
                                    sum(map(float, mapping_summary[i]["unmapped"])),
                                    ".2f",
                                ),
                                format(
                                    sum(map(float, mapping_summary[i]["all"])), ".2f"
                                ),
                                ...
                                mapping_summary[i]["number_of_mapped_baits_with_reads"][
                                    -1
                                ],
                                ...
                                "; ".join(mate_pairs),
                                "; ".join(
                                    mapping_summary[i]["reference_sequence_length"]
                                ),
                    ...
                    writer.writerow(
                        [
                            ...
                            format(
                                sum(map(float, mapping_summary[i]["unmapped"])), ".2f"
                            ),
    ...
    def check_index(self, index_directory, reference_genome):
        ...
        if self.aligner == "bowtie2":
            files = [
                os.path.basename(x)
                for x in glob.glob(os.path.join(os.path.dirname(index_directory), "*"))
            ]
            logger.info(json.dumps(files, indent=2))
            if (
                ("bowtie2.1.bt2" in files)
                and ("bowtie2.2.bt2" in files)
                and ("bowtie2.3.bt2" in files)
                and ("bowtie2.4.bt2" in files)
                and ("bowtie2.rev.1.bt2" in files)
                and ("bowtie2.rev.2.bt2" in files)
            ) == False:
                # create index and save results in ./db from reference genome: (.fasta)
                logger.info(
                    "create index for reference: {} using aligner: {} ".format(
                        reference_genome, self.aligner
                    )
                )
                self.create_index(
                    index_directory=index_directory, reference_genome=reference_genome
                ...
                logger.info(
                    "index already exists for reference: {} using aligner: {}".format(
                        reference_genome, self.aligner
                    )
                )
        elif self.aligner == "kma":
            ...
            if (
                ("kma.comp.b" in files)
                and ("kma.length.b" in files)
                and ("kma.seq.b" in files)
                and ("kma.name" in files)
            ) == False:
                # create index and save results in ./db from reference genome: (.fasta)
                logger.info(
                    "create index for reference: {} using aligner: {} ".format(
            ...
            if (
                ("bwa.amb" in files)
                and ("bwa.ann" in files)
                and ("bwa.bwt" in files)
                and ("bwa.pac" in files)
                and ("bwa.sa" in files)
            ) == False:
                # create index and save results in ./db from reference genome: (.fasta)
                logger.info(
    ...
    def run(self):
        ...
        if self.include_baits == True:
            ...
            logger.info("convert SAM file to BAM file")
            self.convert_sam_to_bam(
                input_sam_file=self.baits_card_sam, output_bam_file=self.baits_card_bam
            )

            os.system(
                "samtools view -F4 --threads {threads} {input_bam} | cut -f 1,2,3 | sort -s -n -k 1,1 > {output_tab}".format(
                    threads=self.threads,
            ...
            self.align_bowtie2(
                reference_genome=self.reference_genome_baits,
                index_directory=self.index_directory_bowtie2_baits,
                output_sam_file=self.output_sam_file_baits,
            )

            logger.info("convert SAM file to BAM file")
            self.convert_sam_to_bam(
                input_sam_file=self.output_sam_file_baits,
                output_bam_file=self.output_bam_file_baits,
            )
