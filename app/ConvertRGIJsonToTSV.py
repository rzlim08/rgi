...
class ConvertJsonToTSV(object):
    ...
    def combine_jsons(self):
        if (
            self.homolog_file is not None
            and self.variant_file is not None
            and self.overexpression_file is not None
            and self.rrna_file is not None
        ):
            data = {}
    ...
    def run(self):
        if os.path.isfile(self.filepath):
            ...
            with open(
                os.path.join(f_path, "{}.txt".format(os.path.splitext(f_name)[0])), "w"
            ) as af:
                ...
                if os.path.isfile(self.filepath):
                    ...
                    for hsp in rgi_data:
                        ...
                        for hit in rgi_data[hsp]:
                            if rgi_data[hsp][hit]["type_match"] == "Perfect":
                                order_perfect.append(
                                    (
                                        hit,
                                        rgi_data[hsp][hit]["bit_score"],
                                        rgi_data[hsp][hit]["perc_identity"],
                                    )
                                )
                        ...
                        if len(order_perfect) > 0:
                            ordered = sorted(
                                order_perfect, key=itemgetter(1, 2), reverse=True
                            ).pop(0)
                        elif len(order_strict) > 0:
                            ordered = sorted(
                                order_strict, key=itemgetter(1, 2), reverse=True
                            ).pop(0)
                        else:
                            ordered = sorted(
                                order_loose, key=itemgetter(1, 2), reverse=True
                            ).pop(0)
                        ...
                        if "ARO_category" in rgi_data[hsp][ordered[0]]:
                            for aroctkey in rgi_data[hsp][hit]["ARO_category"]:
                                cgList.append(
                                    str(
                                        rgi_data[hsp][hit]["ARO_category"][aroctkey][
                                            "category_aro_name"
                                        ]
                        ...
                        if (
                            "orf_dna_sequence_possible"
                            in rgi_data[hsp][ordered[0]].keys()
                        ):
                        ...
                        if dna == 1:
                            if (
                                nudged == True
                                and rgi_data[hsp][ordered[0]]["type_match"] == "Perfect"
                            ):
                            ...
                            if len(rgi_data[hsp]) != 0:
                                if rgi_data[hsp][hit]["model_type_id"] == 41091:
                                    if "snp" in rgi_data[hsp][ordered[0]]:
                                        ...
                                        best_snps = "n/a"
                                        other_snps = "n/a"
                                elif rgi_data[hsp][hit]["model_type_id"] in [
                                    40293,
                                    40295,
                                ]:
                                    if "snp" in rgi_data[hsp][ordered[0]]:
                                ...
                                match_dict[hsp] = [
                                    ...
                                    other_snps,
                                    "; ".join(
                                        rgi_data[hsp][ordered[0]]["ARO_category"][x][
                                            "category_aro_name"
                                        ]
                                        for x in rgi_data[hsp][ordered[0]][
                                            "ARO_category"
                                        ]
                                        if rgi_data[hsp][ordered[0]]["ARO_category"][x][
