...
class CARDkmers(object):
    ...
    def __init__(self, input, bwt, rgi, fasta, k, min, threads, output, local, debug):
        ...
        self.fasta_file = os.path.join(
            self.working_directory, "{}.fasta".format(self.base_name)
        )
        self.output_json_file = os.path.join(
            self.working_directory,
            "{o}_{k}mer_analysis.json".format(o=self.output, k=self.k),
        )
        if self.bwt:
            self.input_bam_file = input
            self.bam_directory = os.path.dirname(self.input_bam_file)
            self.output_allele_summary = os.path.join(
                self.working_directory,
                "{o}_{k}mer_analysis.allele.txt".format(o=self.output, k=self.k),
            )
            self.output_gene_summary = os.path.join(
                self.working_directory,
                "{o}_{k}mer_analysis.gene.txt".format(o=self.output, k=self.k),
            )
        if self.rgi:
            self.input_rgi_file = input
            self.output_rgi_summary = os.path.join(
                self.working_directory,
                "{o}_{k}mer_analysis_rgi_summary.txt".format(o=self.output, k=self.k),
            )
        if self.fasta:
            self.input_fasta_file = input
            self.output_fasta_summary = os.path.join(
                self.working_directory,
                "{o}_{k}mer_analysis_fasta_summary.txt".format(o=self.output, k=self.k),
            )
            ...
            try:
                for key, value in rgi_data.items():
                    if isinstance(value, dict):
                        ...
                        hsp = max(
                            value.keys(), key=(lambda key: value[key]["bit_score"])
                        )
                        ...
                        fasta.write(
                            ">{orf}__{hsp}__{model}__{type_hit}\n{dna}\n".format(
                                orf=contig_id,
                                hsp=hsp,
                                model=model,
                                type_hit=type_hit,
    ...
    def get_bwt_sequences(self):
        ...
        if aligner == "ID:KMA":
            os.system(
                """samtools view -F 4 -F 2048 {bam} | while read line; do awk '{cmd}'; done > {out}""".format(
                    bam=self.input_bam_file,
                    cmd="""{print ">"$1"__"$4"__"$3"__"$5"\\n"$11}""",
                    out=self.fasta_file,
                )
            )
        else:
    ...
    def get_bwt_alignment_data(self, header):
        ...
        if flag.isdigit() is False:
            logger.error(
                "failed to parse BAM file: {}, please check which aligner software was used to produce the BAM file in the RGI BWT step.".format(
                    self.input_bam_file
                )
            )
            exit()
    ...
    def populate_rgi_json(
        self, orf, hsp, model, type_hit, num_kmers, amr_c, tax, gen, o
    ):
        o[orf] = {
            "ORF": orf,
            "contig": orf.split()[0],
            "HSP": hsp,
            "ARO_model": model.replace("_", " "),
            "type_hit": type_hit,
            "#_of_kmers_in_sequence": num_kmers,
            "#_of_AMR_kmers": amr_c,
            "taxonomic_info": tax,
            "genomic_info": gen,
        }
    ...
    def split_fasta(self, file):
        ...
        split_sequences = (
            list(self.chunk_list(iterator, list_size)) if list_size > 0 else [[]]
        )  # returns a list of lists
        logger.info(
            "Using {t} threads to query {s} sequences".format(t=self.threads, s=ns)
        )
        ...
        for entry in fasta:
            ...
            if num_kmers > 1:
                ...
                if all(not tax[k] for k in tax) and all(v == 0 for v in gen.values()):
                    ...
                    if type == "rgi":
                        self.populate_rgi_json(
                            orf_id, hsp, model, type_hit, num_kmers, amr_c, tax, gen, o
                        )
                    elif type == "bwt":
                        self.populate_bwt_json(
                            read, model, num_kmers, amr_c, flag, mapq, tax, gen, o
                        )
                    elif type == "fasta":
    ...
    def get_taxon_data(
        self, name, d, tax
    ):  # name - gene or allele, d - dictionary, tax - species or genus name
        if name not in d:
            d[name] = {}
    ...
    def single_species_bwt(
        self, read, path, allele, ssc_allele, ssp_allele, sscp_allele, ssu_allele
    ):
        if not (all(v == 0 for v in read["genomic_info"].values())):
            if read["genomic_info"]["chr"] > self.min - 1:
                if (
                    read["genomic_info"]["plasmid"]
                    + read["genomic_info"]["chr + plasmid"]
                    > self.min - 1
                ):
    ...
    def single_genus_bwt(
        self, read, genus, allele, sgcp_allele, sgc_allele, sgp_allele, sgu_allele
    ):
        if not (all(v == 0 for v in read["genomic_info"].values())):
            if read["genomic_info"]["chr"] > self.min - 1:
                if (
    ...
    def ambiguous_bwt(
        self, read, allele, cp_allele, a_allele, m_allele, c_allele, taxon
    ):
        # taxon - if
        if not (all(v == 0 for v in read["genomic_info"].values())):
            if read["genomic_info"]["chr"] > self.min - 1:
    ...
    def make_summaries(
        self, d, ssc, sscp, ssp, ssu, sgc, sgcp, sgp, sgu, cd, m, cp, ad
    ):
        summary = []
        for a in d:
            total_hits = 0
            ...
            if a in ssc:
                ...
                ssc_hits = 0
            if a in sscp:
                sscp_str, sscp_hits, sscp_pred_str = self.organize_summary_data(
                    a, sscp, 1
                )
            else:
            ...
            if a in sgc:
                ...
                sgc_hits = 0
            if a in sgcp:
                sgcp_str, sgcp_hits, sgcp_pred_str = self.organize_summary_data(
                    a, sgcp, 1
                )
            else:
                ...
                if j[read]["#_of_kmers_in_sequence"] > 0:
                    ...
                    if len(j[read]["taxonomic_info"]["species"]) == 1:
                        ...
                        if not j[read]["taxonomic_info"]["genus"]:
                            if (
                                j[read]["taxonomic_info"]["species"][path]
                                > self.min - 1
                            ):
                                hits += 1
                                if type == "bwt":
                                    self.single_species_bwt(
                                        j[read],
                                        path,
                                        allele,
                                        ssc_allele,
                            ...
                            if len(j[read]["taxonomic_info"]["genus"]) == 1:
                                genus = set(
                                    j[read]["taxonomic_info"]["genus"].keys()
                                ).pop()
                                if genus == path.split()[0]:
                                    if (
                                        j[read]["taxonomic_info"]["species"][path]
                                        + j[read]["taxonomic_info"]["genus"][genus]
                                        > self.min - 1
                                    ):
                                        ...
                                        if type == "bwt":
                                            ...
                                            prediction = self.single_species_rgi(
                                                j[read], path
                                            )
                                    else:
                                        if type == "bwt":
                                    ...
                                    if (
                                        j[read]["taxonomic_info"]["genus"][genus]
                                        > self.min - 1
                                        and j[read]["taxonomic_info"]["species"][path]
                                        > self.min - 1
                                    ):
                                        hits += 1
                                        if type == "bwt":
                                        ...
                                        if type == "bwt":
                                            ...
                                            prediction = self.single_genus_rgi(
                                                j[read], genus
                                            )
                                    else:
                                        if type == "bwt":
                                ...
                                max_genus = max(
                                    j[read]["taxonomic_info"]["genus"].keys(),
                                    key=(
                                        lambda key: j[read]["taxonomic_info"]["genus"][
                                            key
                                        ]
                                    ),
                                )
                                if (
                                    sum(
                                        int(c) > self.min - 1
                                        for c in j[read]["taxonomic_info"][
                                            "genus"
                                        ].values()
                                    )
                                    > 1
                                    ...
                                    j[read]["taxonomic_info"]["genus"][max_genus]
                                    > self.min - 1
                                ):
                                    if max_genus == path.split()[0]:
                                        if (
                                            j[read]["taxonomic_info"]["species"][path]
                                            + j[read]["taxonomic_info"]["genus"][
                                                max_genus
                                            ]
                                            > self.min - 1
                                        ):
                        ...
                        hits += 1
                        max_species = max(
                            j[read]["taxonomic_info"]["species"].keys(),
                            key=(lambda key: j[read]["taxonomic_info"]["species"][key]),
                        )
                        # multiple single species
                        if (
                            sum(
                                int(c) > self.min - 1
                                for c in j[read]["taxonomic_info"]["species"].values()
                            )
                            > 1
                        ):
                            passing_species = [
                                path
                                for path in j[read]["taxonomic_info"]["species"]
                                if j[read]["taxonomic_info"]["species"][path]
                                > self.min - 1
                            ]
                                ...
                                if not j[read]["taxonomic_info"]["genus"]:
                                    ...
                                    if len(j[read]["taxonomic_info"]["genus"]) == 1:
                                        ...
                                        max_genus = max(
                                            j[read]["taxonomic_info"]["genus"].keys(),
                                            key=(
                                                lambda key: j[read]["taxonomic_info"][
                                                    "genus"
                                                ][key]
                                            ),
                                        )
                            ...
                            j[read]["taxonomic_info"]["species"][max_species]
                            > self.min - 1
                        ):
                            if not j[read]["taxonomic_info"]["genus"]:
                                if type == "bwt":
                        ...
                        if len(j[read]["taxonomic_info"]["genus"]) == 1:
                            ...
                            max_genus = max(
                                j[read]["taxonomic_info"]["genus"].keys(),
                                key=(
                                    lambda key: j[read]["taxonomic_info"]["genus"][key]
                                ),
                            )
                            if (
                                sum(
                                    int(c) > self.min - 1
                                    for c in j[read]["taxonomic_info"]["genus"].values()
                                )
                                > 1
                            ):
                                if type == "bwt":
    ...
    def make_bwt_summary(
        ...
        # make summaries
        allele_summary = self.make_summaries(
            all_alleles,
            ssc_allele,
            sscp_allele,
            ssp_allele,
        ...
        gene_summary = self.make_summaries(
            all_genes,
            ssc_gene,
            sscp_gene,
            ssp_gene,
    ...
    def run(self):
        ...
        if self.rgi:
            ...
            num_seq_total, short_total, o_total = self.query_sequences(
                self.k, j, amr_kmers, iterator, "rgi"
            )
        ...
        if self.bwt:
            (
                ...
                cp_allele,
                a_allele,
            ) = self.parse_kmer_json("bwt")

            self.make_bwt_summary(
                all_alleles,
                ssc_allele,
                ssp_allele,
                sscp_allele,
