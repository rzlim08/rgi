...
class Database(object):
    ...
    def create_blast_config(self):
        ...
        os.system(
            "echo 'export BLAST_USAGE_REPORT=false' >> {config_file}".format(
    ...
    def make_blast_database(self):
        ...
        if (
            os.path.isfile(os.path.join(self.db, "proteindb.fsa")) == True
            and os.path.exists(os.path.join(self.db, "proteindb.fsa")) == True
            and os.path.exists(os.path.join(self.db, "protein.db.phr")) == True
            and os.path.exists(os.path.join(self.db, "protein.db.pin")) == True
            ...
            os.system(
                "makeblastdb -in {} -dbtype prot -out {} {stdout}".format(
                    os.path.join(self.db, "proteindb.fsa"),
    ...
    def make_diamond_database(self):
        ...
        if (
            ...
            os.system(
                "diamond makedb --quiet --in {} --db {} {stdout}".format(
                    os.path.join(self.db, "proteindb.fsa"),
                    os.path.join(self.db, "protein.db"),
                    stdout=self.stdout,
                )
    ...
    def make_custom_db(self, in_file, out_file, db_type="nucl", program="blast"):
        if program == "blast":
            os.system(
                "makeblastdb -in {in_file}  -dbtype {db_type} -out {out_file} \
				{stdout}".format(
                    in_file=in_file,
                    db_type=db_type,
                    out_file=out_file,
                ...
                for i in j:
                    if i.isdigit():
                        ...
                        if j[i]["model_type_id"] == "40292":
                            try:
                                pass_bit_score = j[i]["model_param"][
                                    "blastp_bit_score"
                                ]["param_value"]
                            except KeyError:
                                logger.warning(
                                    "No bitscore for model (%s, %s). RGI will omit this model and keep running."
                                ...
                                try:
                                    for seq in j[i]["model_sequences"]["sequence"]:
                                        fout.write(
                                            ">%s_%s | model_type_id: 40292 | pass_bitscore: %s | %s\n"
                                            % (i, seq, pass_bit_score, j[i]["ARO_name"])
                                        )
                                        fout.write(
                                            "%s\n"
                                            % (
                                                j[i]["model_sequences"]["sequence"][
                                                    seq
                                    ...
                                    logger.warning(
                                        "No model sequences for model (%s, %s). RGI will omit this model and keep running."
                                        % (j[i]["model_id"], j[i]["model_name"])
                                    )
                                    logger.info(
                                        "Please let the CARD Admins know! Email: card@mcmaster.ca"
                            ...
                            try:
                                ...
                                try:
                                    snpList = [
                                        j[i]["model_param"]["snp"]["param_value"][k]
                                        for k in j[i]["model_param"]["snp"][
                                            "param_value"
                                        ]
                                    ]
                                except Exception as e:
                                    logger.warning(
                                        "No snp for model (%s, %s). RGI will omit this model and keep running."
                                        % (j[i]["model_id"], j[i]["model_name"])
                                    )
                                    logger.info(
                                        "Please let the CARD Admins know! Email: card@mcmaster.ca"
                                ...
                                try:
                                    for seq in j[i]["model_sequences"]["sequence"]:
                                        fout.write(
                                            ">%s_%s | model_type_id: 40293 | pass_bit_score: %s | SNP: %s | %s\n"
                                            % (
                                                i,
                                                seq,
                                                pass_bit_score,
                            ...
                            try:
                                ...
                                try:
                                    for seq in j[i]["model_sequences"]["sequence"]:
                                        fout.write(
                                            ">%s_%s | model_type_id: 41091 | pass_bit_score: %s | SNP: %s | %s\n"
                                            % (
                                                i,
                                                seq,
                                                pass_bit_score,
                        ...
                        if j[i]["model_type_id"] == "40295":
                            try:
                                ...
                                if snpList:
                                    for seq in j[i]["model_sequences"]["sequence"]:
                                        if (
                                            j[i]["model_sequences"]["sequence"][seq][
                                                "dna_sequence"
                                            ]["strand"]
                                            == "-"
                                        ):
                                            ...
                                            fout.write(
                                                ">%s_%s | model_type_id: 40295 | pass_bit_score: %s | SNP: %s | %s\n"
                                                % (
                                                    i,
                                                    seq,
                                                    pass_bit_score,
