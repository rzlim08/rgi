...
def test_create_local_db(rgi):
    ...
    rgi.load_run(parser.parse_args(["--card_json", f, "--local", "--debug"]))

    assert os.path.isfile(f) and os.path.exists(f), print(
        "add card.json to {} directory and re-run test".format(
