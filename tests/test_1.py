...
def test_rgi_protein_sequence(rgi):
    ...
    run_rgi(
        rgi, "protein", os.path.join(working_directory, inputs, filename), output_file
    )
...
def test_rgi_nucleotide_sequence(rgi):
    ...
    output_file = os.path.join(working_directory, outputs, "{}.json".format(filename))
    run_rgi(
        rgi, "contig", os.path.join(working_directory, inputs, filename), output_file
    )
...
def test_rgi_effluxpump_model(rgi):
    ...
    assert (
        validate_results(output_file, 100, "Pseudomonas aeruginosa CpxR", "Perfect")
        == True
    )
