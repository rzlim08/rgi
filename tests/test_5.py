...
def test_rgi_bwt_bwa(rgi):
    ...
    run_rgi(
        rgi,
        os.path.join(working_directory, inputs, read_one),
        os.path.join(working_directory, inputs, read_two),
        "bwa",
        "3",
        output_file,
    )
    gene = sp.getoutput(
        "cat {gene_summary_output} | cut -f1 | grep '{amr_gene}'".format(
...
def test_rgi_bwt_bwa_single(rgi):
    ...
    output_file = os.path.join(working_directory, outputs, "{}".format(filename))
    run_rgi_single(
        rgi, os.path.join(working_directory, inputs, read_one), "bwa", "3", output_file
    )
    gene = sp.getoutput(
        "cat {gene_summary_output} | cut -f1 | grep '{amr_gene}'".format(
            gene_summary_output="{}.gene_mapping_data.txt".format(output_file),
...
def test_rgi_bwt_kma_interleaved(rgi):
    ...
    output_file = os.path.join(working_directory, outputs, "{}".format(filename))
    run_rgi_single(
        rgi, os.path.join(working_directory, inputs, read_one), "kma", "3", output_file
    )
    gene = sp.getoutput(
        "cat {gene_summary_output} | cut -f1 | grep '{amr_gene}'".format(
            gene_summary_output="{}.gene_mapping_data.txt".format(output_file),
